...
Profiler::Profiler() {
  ...
  size_t num_step_logs = RunConfig::num_global_step_per_epoch * RunConfig::num_epoch;
  ...
  // _num_step = RunConfig::num_global_step_per_epoch;

  if (RunConfig::option_log_node_access || RunConfig::option_log_node_access_simple) {
    _node_access.resize(RunConfig::num_total_item, 0);
    _last_visit.resize(RunConfig::num_total_item, 0);
    _similarity.resize(num_step_logs);
    _epoch_last_visit.resize(RunConfig::num_total_item, 0);
    _epoch_cur_visit.resize(RunConfig::num_total_item, 0);
...
void Profiler::LogEpochAdd(uint64_t key, LogEpochItem item, double val) {
  uint64_t epoch = RunConfig::GetEpochFromKey(key);
  size_t item_idx = static_cast<size_t>(item);
...
void Profiler::ReportStepAverageLastEpoch(uint64_t epoch, uint64_t step) {
  uint64_t key = RunConfig::GetBatchKey(epoch, step);
  if (key < RunConfig::num_global_step_per_epoch) {
    return;
  }
  ...
  for (size_t i = 0; i < num_items; i++) {
    double sum = 0;
    size_t cnt = 0;
    for (size_t current_key = key - RunConfig::num_global_step_per_epoch; current_key < key; current_key++) {
      if (_step_data[i].bitmap[current_key] == false) continue;
      sum += _step_data[i].vals[current_key];
      cnt ++;
    }
...
#pragma omp parallel for num_threads(RunConfig::omp_thread_num)
  ...
  if (RunConfig::GetStepFromKey(key) == RunConfig::num_global_step_per_epoch-1) {
...
#pragma omp parallel for num_threads(RunConfig::omp_thread_num) reduction(+ : e_min_freq_both_e_top_K, total_cur_e_top_K_freq)
    ...
    _epoch_similarity[RunConfig::GetEpochFromKey(key)] = e_min_freq_both_e_top_K/(double)total_cur_e_top_K_freq*100;
...
void Profiler::ReportNodeAccess() {
  ...
  double num_nodes =
      static_cast<double>(RunConfig::num_total_item);
...
#pragma omp parallel for num_threads(RunConfig::omp_thread_num)
  ...
  for (auto & p : records) {
    ...
    float avg_frequency = frequency / static_cast<double>(RunConfig::num_epoch);
    IdType nodeid = p.second;
  ...
  for (int cache_rate = 0; cache_rate <= 100; cache_rate++) {
    ...
    ofs0 << cache_rate << "\t" << ((idx == 0) ? (double)0 : (records[idx].first - records[idx-1].first) / (double)RunConfig::num_epoch) << "\n";
