...
def ds_load(worker_id, run_config):
    ...
    print(train_data_list[0][0].shape, train_data_list[0][1].shape)
    orig_size = train_data_list[0][0].shape[0]
    pad_size = int(orig_size * 0.05)
    pad_size = (pad_size + 7) - (pad_size + 7) % 8
    first_batch_index = torch.cat((train_data_list[0][0], train_data_list[1][0][:pad_size]))
    first_batch_label = torch.cat((train_data_list[0][1], train_data_list[1][1][:pad_size]))
    train_data_list[0] = (first_batch_index, first_batch_label)
    # train_data_list[0][0] = torch.cat((train_data_list[0][0], train_data_list[1][0][:int(orig_size * 0.05)]))
    # train_data_list[0][1] = torch.cat((train_data_list[0][1], train_data_list[1][1][:int(orig_size * 0.05)]))
    print(train_data_list[0][0].shape, train_data_list[0][1].shape)
...
def main(worker_id, run_config):
    ...
    model = GCN(in_feat, run_config["hiddensize"], num_class,
    ...
    for epoch in range(num_epoch):
        ...
        for i, (idx, batch_label) in enumerate(train_data_list):
            ...
            step_key += num_worker
            if worker_id == 0:
                print(f"[Epoch {epoch}][Step {i}], time={step_end_time - step_start_time}, ext_time={extract_end_time - sample_end_time}, train_time={step_end_time - extract_end_time}")
        global_barrier.wait()
