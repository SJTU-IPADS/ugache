...
def inference_with_saved_model(args):
    ...
    with strategy.scope():
        if args["coll_cache_policy"] == "sok":
            ...
            collcache_tf2.Config(global_batch_size = args["global_batch_size"],
    ...
    # @tf.function
    def _record_hotness(inputs, labels):
        sparse_keys = inputs[0]
    ...
    for i in range(args["coll_cache_enable_iter"]):
        ret = strategy.run(_record_hotness, args=(ret_list[i]))
...
if args["coll_cache_policy"] == "sok":
    ...
    Shutdown = collcache_tf2.Shutdown
